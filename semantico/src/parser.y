...
%type    <decl_f> decl_w_args
%type    <decl_f> decl_no_args
%type    <fun_f> fundecl  
...
procdecl:
    ...
    | decl_w_args DO '{' stmts '}' where_exp { 
        proc_t* tmp = NEW(proc_t);
        decl_t* d = $1;
        tmp->label = d->label;
        tmp->args = d->args;
    ...
    | decl_no_args DO '{' stmts '}' where_exp  { 
        proc_t* tmp = NEW(proc_t);
        decl_t* d = $1;
        tmp->label = d->label;
        tmp->stmts = $4;
